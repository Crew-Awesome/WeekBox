...
export class GameBananaTransport {
  constructor({ baseUrl, fetchImpl = (...args) => fetch(...args), config = {} }) {
  ...
  async getModIndex({ gameId, categoryId, page, perPage, sort, signal }) {
    ...
    if (cached && Date.now() - cached.at < this.config.transportCacheFreshMs) return cached.data;
    if (this.inFlight.has(url)) return this.inFlight.get(url);
    const request = this.request(url, signal).then((data) => {
      this.cache.set(url, { at: Date.now(), data }); return data;
  ...
  async request(url, signal) {
    ...
    const timeout = setTimeout(() => controller.abort(), this.config.requestTimeoutMs || 12000);
    try {
      let attempt = 0;
      while (true) {
        let response;
        try { response = await this.fetchImpl(url, { signal: controller.signal, headers: { 'Accept': 'application/json' } }); }
          ...
          throw new GameBananaTransportError("network", "Category request failed", { cause });
        }
        if (!response?.ok) {
          const retryAfter = response.headers?.get?.("Retry-After");
          throw new GameBananaTransportError("http", "Category request failed", { status: response?.status, retryAfter });
        }
        try { return await response.json(); }
        catch (cause) { throw new GameBananaTransportError("parse", "Category response was invalid", { cause }); }
      }
    } finally {
