window.HomeSearchDropdown = {
    ...
    saveRecent(query) {
        if(!query) return;
        // Evitamos duplicados moviendo el reciente al principio
    ...
    async updateDropdown() {
        ...
        // 1. Mostrar recientes filtradas por lo que se está escribiendo
        ...
        // 2. Mostrar búsquedas relacionadas usando GameBanana API
        if(query.length > 2) {
    ...
    async fetchRelated(query) {
        try {
            // Obtenemos solo 3 resultados rápidos para sugerencias
