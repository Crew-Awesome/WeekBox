// public/launcher/src/utils/FS.js
...
class FileSystemService extends EventTarget {
    ...
    async installEngine(engineId, engineName, version, downloadUrl) {
        ...
        try {
            ...
            const MAX_BUFFER_SIZE = 1024 * 1024; // 1MB buffer para no bloquear el Event Loop
            ...
            while (true) {
                ...
                if (writeBufferSize >= MAX_BUFFER_SIZE) {
                    await flushBuffer();
                    // YIELD ARCHITECTURAL: Cede el hilo principal para que UI no se congele
...
// Exportamos la instancia (Singleton Pattern)
export const FS = new FileSystemService();
// Retrocompatibilidad estricta para módulos no refactorizados
window.GenesisFS = FS;